...
describe('product portfolio', () => {
  ...
  it('uses a parent-brand homepage, not a fifth product sales page', () => {
    ...
    const header = readFileSync(join(root, 'components/Header.tsx'), 'utf8')
    assert.match(home, /Engineering systems should know/)
    assert.match(home, /more than how to run/)
    assert.match(home, /See the products/)
    assert.match(home, /Talk to OpsDevCode/)
    assert.match(home, /Four products/)
    assert.match(home, /Four engineering domains/)
    ...
    assert.match(home, /Dispatch/)
    assert.match(home, /PRODUCT_URLS/)
    assert.match(home, /href="\/architecture"/)
    assert.match(home, /variant="portfolio"/)
    assert.doesNotMatch(home, /THESIS/)
    assert.doesNotMatch(home, /PARENT SYSTEM/)
    assert.doesNotMatch(home, /Start free/)
    assert.doesNotMatch(home, /Try it now/)
    assert.doesNotMatch(home, /Deploy the platform/)
    assert.doesNotMatch(home, /Connect your cloud/)
    assert.doesNotMatch(home, /See all four working together/)
    assert.doesNotMatch(home, /Explore the system/)
    assert.doesNotMatch(home, /SystemMap/)
    assert.doesNotMatch(home, /problem-planes/)
    assert.doesNotMatch(home, /variant="hero"/)
    assert.doesNotMatch(home, /variant="family"/)
    ...
    assert.match(header, /label: 'Talk'/)
    assert.doesNotMatch(header, /Convergence/)
  ...
  it('keeps Convergence off the product hierarchy on the homepage', () => {
    const home = readFileSync(join(root, 'src/app/page.tsx'), 'utf8')
    const footer = readFileSync(join(root, 'components/Footer.tsx'), 'utf8')
    assert.match(home, /independent research/)
    assert.match(home, /not an OpsDevCode\s+product/)
    assert.doesNotMatch(home, /slug: 'convergence'/)
    assert.match(footer, /CONVERGENCE_URL/)
  })
})
